...
import FUTURE.server2.run_server_2 as future
    ...
    with WORKER_PRELOAD_LOCK:
        ...
        def runner() -> None:
            try:
                ...
                print(f"Future STT worker 2 ready: {future.SERVER_STATE.get('model_ref', '')}", flush=True)
            except Exception as exc:
                future.SERVER_STATE.update({"ready": False, "loading": False, "last_error": str(exc)})
                print(f"Future STT worker 2 preload failed: {exc}", flush=True)

        WORKER_PRELOAD_THREAD = threading.Thread(target=runner, name="future-stt-worker-2-preload", daemon=True)
        WORKER_PRELOAD_THREAD.start()
        return True


class FutureSttWorkerHandler(BaseHTTPRequestHandler):
    server_version = "FutureSttWorker2/1.0"
    ...
    def log_message(self, fmt, *args):
        print(f"[STT2 {time.strftime('%H:%M:%S')}] {self.address_string()} {fmt % args}", flush=True)


def main() -> int:
    future.set_future_server2_process_status("stt-worker")
    parser = argparse.ArgumentParser(description="Future persistent STT worker 2")
    ...
    print(f"Future STT worker 2 listening at http://{args.host}:{args.port}", flush=True)
    try:
        httpd.serve_forever()
    finally:
        httpd.server_close()
