...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .audio-cache-action {{
      color: #fff7e6;
      border-color: rgba(255, 190, 96, 0.58);
      background:
        radial-gradient(circle at 18% 18%, rgba(255, 255, 255, 0.28), transparent 28%),
        linear-gradient(135deg, #ef8d4d, #d94d76 56%, #8f5de8);
      box-shadow: 0 0 28px rgba(239, 141, 77, 0.2);
      position: relative;
      overflow: hidden;
    }}
    .audio-cache-action::after {{
      content: "";
      position: absolute;
      inset: 1px;
      ...
      pointer-events: none;
      animation: audioCacheActionPulse 2.8s ease-in-out infinite;
    }}
    .audio-cache-action:disabled {{ opacity: 0.64; cursor: wait; }}
    .copy {{
      margin-top: 12px;
      min-height: 38px;
    ...
    @keyframes audioCacheActionPulse {{
...
<body>
  <main>
    <section class="top">
      <div>
        ...
        <div class="actions">
          ...
          <button class="action audio-cache-action" id="frontend-clear-audio-cache" type="button">Clear audio cache</button>
    ...
    <section class="grid tab-panel" id="panel-main">
      <div class="card wide">
        <div class="label">Space_V picture folder</div>
        <div class="setting-copy">Only local images are used. Each filename (without extension) must match the English vocabulary word.</div>
        <div class="notice-actions">
          <input class="setting-input" id="space-v-picture-folder-input" type="text" maxlength="1024" spellcheck="false" value="C:\\server data\\Picture\\picture" aria-label="Space V picture folder">
          <button class="notice-save" id="space-v-picture-folder-save" type="button">Save</button>
        </div>
        <span class="notice-state" id="space-v-picture-folder-state">Loading local picture index...</span>
      </div>
      <div class="card">
      ...
      <div class="card wide">
        ...
        <div class="setting-editor">
          ...
          <div class="setting-row">
            <div class="setting-copy">
              English QmDict audio (UK + US). Force overwrites every file; Missing only downloads absent files; Resume continues the last interrupted session.
            </div>
            <div class="cpu-guard-control">
              <button class="notice-save cpu-guard-apply" id="qmdict-word-audio-force" type="button">Force Reload All UK/US</button>
              <button class="notice-save cpu-guard-apply" id="qmdict-word-audio-missing" type="button">Check Missing &amp; Download</button>
              <button class="notice-save cpu-guard-apply" id="qmdict-word-audio-resume" type="button">Resume UK/US Session</button>
              <button class="notice-save cpu-guard-apply" id="qmdict-word-audio-cancel" type="button">Stop UK/US</button>
            </div>
            <div class="qmdict-audio-progress" id="qmdict-word-audio-progress" hidden>
              <span class="qmdict-audio-bar"><i id="qmdict-word-audio-progress-fill"></i></span>
              <span class="qmdict-audio-copy" id="qmdict-word-audio-progress-copy">UK/US audio refresh is idle.</span>
            </div>
            <div class="setting-copy">
              Vietnamese QmDict meaning audio. Force overwrites every Vietnamese file; Missing only fills gaps; Resume continues its own saved session.
            </div>
            <div class="cpu-guard-control">
              <button class="notice-save cpu-guard-apply" id="qmdict-meaning-audio-force" type="button">Force Reload All Vietnamese</button>
              <button class="notice-save cpu-guard-apply" id="qmdict-meaning-audio-missing" type="button">Check Missing &amp; Download</button>
              <button class="notice-save cpu-guard-apply" id="qmdict-meaning-audio-resume" type="button">Resume Vietnamese Session</button>
              <button class="notice-save cpu-guard-apply" id="qmdict-meaning-audio-cancel" type="button">Stop Vietnamese</button>
            </div>
            <div class="qmdict-audio-progress" id="qmdict-meaning-audio-progress" hidden>
              <span class="qmdict-audio-bar"><i id="qmdict-meaning-audio-progress-fill"></i></span>
              <span class="qmdict-audio-copy" id="qmdict-meaning-audio-progress-copy">Vietnamese audio refresh is idle.</span>
            </div>
            <div class="cpu-guard-control">
              <button class="notice-save cpu-guard-apply" id="qmdict-reload" type="button">Reload QmDict Dictionary</button>
            </div>
          ...
          <div class="setting-row">
            ...
            <div class="cpu-guard-control">
              <button class="notice-save cpu-guard-apply" id="server-data-manifest-refresh" type="button">Apply pending manifest updates</button>
            </div>
  ...
  <script>
    ...
    const frontendClearAudioCacheButton = document.getElementById("frontend-clear-audio-cache");
    ...
    const qmdictWordAudioForce = document.getElementById("qmdict-word-audio-force");
    const qmdictWordAudioMissing = document.getElementById("qmdict-word-audio-missing");
    const qmdictWordAudioResume = document.getElementById("qmdict-word-audio-resume");
    const qmdictWordAudioCancel = document.getElementById("qmdict-word-audio-cancel");
    const qmdictMeaningAudioForce = document.getElementById("qmdict-meaning-audio-force");
    const qmdictMeaningAudioMissing = document.getElementById("qmdict-meaning-audio-missing");
    const qmdictMeaningAudioResume = document.getElementById("qmdict-meaning-audio-resume");
    const qmdictMeaningAudioCancel = document.getElementById("qmdict-meaning-audio-cancel");
    ...
    const spaceVPictureFolderInput = document.getElementById("space-v-picture-folder-input");
    const spaceVPictureFolderSave = document.getElementById("space-v-picture-folder-save");
    const spaceVPictureFolderState = document.getElementById("space-v-picture-folder-state");
    const qmdictWordAudioProgress = document.getElementById("qmdict-word-audio-progress");
    const qmdictWordAudioProgressFill = document.getElementById("qmdict-word-audio-progress-fill");
    const qmdictWordAudioProgressCopy = document.getElementById("qmdict-word-audio-progress-copy");
    const qmdictMeaningAudioProgress = document.getElementById("qmdict-meaning-audio-progress");
    const qmdictMeaningAudioProgressFill = document.getElementById("qmdict-meaning-audio-progress-fill");
    const qmdictMeaningAudioProgressCopy = document.getElementById("qmdict-meaning-audio-progress-copy");
  ...
  const onlineUsersNode = document.getElementById("online-users");
    ...
    const render = (state) => {{
      const buildHold = state && state.manifest_build_hold ? state.manifest_build_hold : {{}};
      if (serverDataManifestRefresh) {{
        serverDataManifestRefresh.disabled = Boolean(buildHold.active_builders || buildHold.publish_in_progress || buildHold.active);
      }}
      if (serverDataManifestState && (buildHold.requires_apply || Number(buildHold.pending_paths || 0) > 0)) {{
        const count = Number(buildHold.pending_paths || 0).toLocaleString();
        serverDataManifestState.textContent = buildHold.active
          ? `Builder active: ${{count}} pending paths; active users stay on the previous manifest.`
          : `Ready to apply ${{count}} staged builder paths.`;
      }}
    ...
    const renderQmDictStatus = (reload = {{}}, audio = {{}}, repair = {{}}, wordAudio = {{}}) => {{
      ...
      const wordPhase = String(wordAudio && wordAudio.phase || "");
      const wordScanTotal = Number(wordAudio && wordAudio.scan_total || 0) || 0;
      const wordScanDone = Number(wordAudio && wordAudio.scan_done || 0) || 0;
      const wordPriorityTotal = Number(wordAudio && wordAudio.priority_total || 0) || 0;
      const wordPriorityDone = Number(wordAudio && wordAudio.priority_done || 0) || 0;
      ...
      const wordActiveTotal = ["prioritizing", "scanning"].includes(wordPhase) ? wordScanTotal : wordAudioTotal;
      const wordActiveDone = ["prioritizing", "scanning"].includes(wordPhase) ? wordScanDone : wordAudioDone;
      const wordAudioPercent = wordActiveTotal > 0 ? Math.max(0, Math.min(100, Math.round((wordActiveDone / wordActiveTotal) * 100))) : (wordAudioRunning ? 1 : 0);
      const percent = repairRunning ? repairPercent : audioPercent;
      ...
      const wordAudioCopy = wordAudioRunning
        ? wordPhase === "prioritizing"
          ? ` | OCR priority scan ${{wordScanDone}}/${{wordScanTotal || "..."}} tokens | matched ${{Math.floor(wordPriorityTotal / 2)}} words`
          : wordPhase === "scanning"
            ? ` | checking UK/US files ${{wordScanDone}}/${{wordScanTotal || "..."}}`
            : ` | refreshing UK/US audio ${{wordAudioDone}}/${{wordAudioTotal || "..."}} | OCR priority ${{wordPriorityDone}}/${{wordPriorityTotal || 0}}`
        : wordAudioTotal
      ...
      if (qmdictWordAudioProgress) {{
        qmdictWordAudioProgress.hidden = !(wordAudioRunning || wordAudioTotal > 0 || String(wordAudio && wordAudio.completed_at || ""));
        qmdictWordAudioProgress.classList.toggle("is-running", wordAudioRunning);
      }}
      if (qmdictWordAudioProgressFill) qmdictWordAudioProgressFill.style.width = `${{wordAudioPercent}}%`;
      if (qmdictWordAudioProgressCopy) {{
        qmdictWordAudioProgressCopy.textContent = wordAudioRunning
          ? wordPhase === "prioritizing"
            ? `OCR priority scan: ${{wordScanDone}}/${{wordScanTotal || "..."}} tokens (${{wordAudioPercent}}%) | matched ${{Math.floor(wordPriorityTotal / 2)}} words`
            : wordPhase === "scanning"
              ? `Checking UK/US files: ${{wordScanDone}}/${{wordScanTotal || "..."}} (${{wordAudioPercent}}%) | missing ${{wordAudioTotal}}`
              : `UK/US session: ${{wordAudioDone}}/${{wordAudioTotal || "..."}} (${{wordAudioPercent}}%) | OCR priority ${{wordPriorityDone}}/${{wordPriorityTotal || 0}} | SOT slots ${{wordAudioWorkers || "..."}} | refreshed ${{wordAudioRefreshed}} | failed ${{wordAudioFailed}}${{wordLastCopy}}`
          : wordAudioTotal > 0
            ? `UK/US session finished: refreshed ${{wordAudioRefreshed}}/${{wordAudioTotal}} | failed ${{wordAudioFailed}}${{wordLastCopy}}`
            : "UK/US audio refresh is idle.";
      }}
      if (qmdictMeaningAudioProgress) {{
        qmdictMeaningAudioProgress.hidden = !(running || scheduled || total > 0 || scanTotal > 0 || String(audio && audio.completed_at || ""));
        qmdictMeaningAudioProgress.classList.toggle("is-running", running || scheduled);
      }}
      if (qmdictMeaningAudioProgressFill) qmdictMeaningAudioProgressFill.style.width = `${{audioPercent}}%`;
      if (qmdictMeaningAudioProgressCopy) {{
        const last = String(audio && audio.last_meaning || "");
        qmdictMeaningAudioProgressCopy.textContent = running
          ? phase === "scanning"
            ? `Vietnamese check: ${{scanDone}}/${{scanTotal || "..."}} (${{audioPercent}}%) | cached ${{cached}} | missing ${{missing}}${{last ? ` | ${{last}}` : ""}}`
            : `Vietnamese session: ${{done}}/${{total || "..."}} (${{audioPercent}}%) | workers ${{workers || "..."}} | cached ${{cached}} | downloaded ${{downloaded}} | failed ${{failed}}${{last ? ` | ${{last}}` : ""}}`
          : scanTotal > 0
            ? `Vietnamese session finished: scanned ${{scanTotal}} | cached ${{cached}} | downloaded ${{downloaded}} | failed ${{failed}}`
    ...
    const renderSpaceVPictureSettings = (payload = {{}}) => {{
      const folder = String(payload.folder || "");
      if (spaceVPictureFolderInput && document.activeElement !== spaceVPictureFolderInput) {{
        spaceVPictureFolderInput.value = folder;
      }}
      if (spaceVPictureFolderState) {{
        const indexed = Math.max(0, Number(payload.matched_words || payload.indexed_files || 0) || 0);
        const duplicate = Math.max(0, Number(payload.duplicate_words || 0) || 0);
        const ignored = Math.max(0, Number(payload.ignored_files || 0) || 0);
        spaceVPictureFolderState.textContent = payload.exists
          ? `Local index ready: ${{indexed.toLocaleString()}} words | duplicates ${{duplicate.toLocaleString()}} | ignored ${{ignored.toLocaleString()}}.`
          : `Picture folder does not exist: ${{folder || "not configured"}}`;
      }}
    }};
    const loadSpaceVPictureSettings = async () => {{
      try {{
        const response = await fetch("/dashboard/space-v-picture-settings?ts=" + Date.now(), {{ cache: "no-store" }});
        const payload = await response.json().catch(() => ({{}}));
        if (!response.ok || payload.ok === false) {{
          throw new Error(payload.error || "Could not load the Space_V picture folder.");
        }}
        renderSpaceVPictureSettings(payload);
      }} catch (error) {{
        if (spaceVPictureFolderState) spaceVPictureFolderState.textContent = String(error && error.message ? error.message : error);
      }}
    }};
    const loadSettings = async () => {{
      try {{
        void loadSpaceVPictureSettings();
    ...
    if (frontendClearAudioCacheButton) {{
      frontendClearAudioCacheButton.addEventListener("click", async () => {{
        frontendClearAudioCacheButton.disabled = true;
        const previousLabel = frontendClearAudioCacheButton.textContent;
        frontendClearAudioCacheButton.textContent = "Clearing...";
        setTextIfChanged(messageNode, "Broadcasting audio-cache clear command to connected clients...");
        try {{
          const response = await fetch("/frontend-version/bump", {{
            method: "POST",
            headers: {{ "Content-Type": "application/json" }},
            body: JSON.stringify({{
              reason: "dashboard_audio_cache_clear",
              command: "clear-audio-cache",
            }}),
          }});
          const payload = await response.json().catch(() => ({{}}));
          if (!response.ok || payload.ok === false) {{
            throw new Error(payload.error || "Could not clear connected client audio caches.");
          }}
          setTextIfChanged(messageNode, "Audio-cache clear command sent to connected clients.");
        }} catch (error) {{
          setTextIfChanged(messageNode, String(error && error.message ? error.message : error));
        }} finally {{
          frontendClearAudioCacheButton.disabled = false;
          frontendClearAudioCacheButton.textContent = previousLabel || "Clear audio cache";
        }}
      }});
    }}
    const startQmDictWordAudioRefresh = async (mode = "all", button = null) => {{
      ...
      if (qmdictReloadState) {{
        qmdictReloadState.textContent = mode === "resume"
          ? "Resuming the previous UK/US audio session..."
          : mode === "missing"
            ? "Checking and downloading missing UK/US audio..."
            : "Force reloading all UK/US audio...";
      }}
      try {{
    ...
    const startQmDictMeaningAudioRefresh = async (mode = "missing", button = null) => {{
      if (button) button.disabled = true;
      if (qmdictReloadState) {{
        qmdictReloadState.textContent = mode === "resume"
          ? "Resuming the previous Vietnamese audio session..."
          : mode === "force"
            ? "Force reloading all Vietnamese audio..."
            : "Checking and downloading missing Vietnamese audio...";
      }}
      try {{
    ...
    const cancelQmDictAudioRefresh = async (kind, button = null) => {{
      if (button) button.disabled = true;
      try {{
        const response = await fetch(`/qmdict/${{kind}}-audio-refresh/cancel`, {{ method: "POST", headers: {{ "Content-Type": "application/json" }}, body: "{{}}" }});
        const payload = await response.json().catch(() => ({{}}));
        if (!response.ok || payload.ok === false) throw new Error(payload.error || "Could not stop the audio session.");
        if (qmdictReloadState) qmdictReloadState.textContent = "Stopping audio session; its checkpoint is being saved...";
        scheduleQmDictStatusPoll();
      }} catch (error) {{
        if (qmdictReloadState) qmdictReloadState.textContent = String(error && error.message ? error.message : error);
      }} finally {{
    ...
    if (qmdictWordAudioForce) qmdictWordAudioForce.addEventListener("click", () => void startQmDictWordAudioRefresh("force", qmdictWordAudioForce));
    if (qmdictWordAudioMissing) qmdictWordAudioMissing.addEventListener("click", () => void startQmDictWordAudioRefresh("missing", qmdictWordAudioMissing));
    if (qmdictWordAudioResume) qmdictWordAudioResume.addEventListener("click", () => void startQmDictWordAudioRefresh("resume", qmdictWordAudioResume));
    if (qmdictWordAudioCancel) qmdictWordAudioCancel.addEventListener("click", () => void cancelQmDictAudioRefresh("word", qmdictWordAudioCancel));
    if (qmdictMeaningAudioForce) qmdictMeaningAudioForce.addEventListener("click", () => void startQmDictMeaningAudioRefresh("force", qmdictMeaningAudioForce));
    if (qmdictMeaningAudioMissing) qmdictMeaningAudioMissing.addEventListener("click", () => void startQmDictMeaningAudioRefresh("missing", qmdictMeaningAudioMissing));
    if (qmdictMeaningAudioResume) qmdictMeaningAudioResume.addEventListener("click", () => void startQmDictMeaningAudioRefresh("resume", qmdictMeaningAudioResume));
    if (qmdictMeaningAudioCancel) qmdictMeaningAudioCancel.addEventListener("click", () => void cancelQmDictAudioRefresh("meaning", qmdictMeaningAudioCancel));
    if (spaceVPictureFolderSave) {{
      spaceVPictureFolderSave.addEventListener("click", async () => {{
        spaceVPictureFolderSave.disabled = true;
        if (spaceVPictureFolderState) spaceVPictureFolderState.textContent = "Saving and rebuilding the local picture index...";
        try {{
          const response = await fetch("/dashboard/space-v-picture-settings", {{
            method: "POST",
            headers: {{ "Content-Type": "application/json" }},
            body: JSON.stringify({{ folder: String(spaceVPictureFolderInput && spaceVPictureFolderInput.value || "").trim() }}),
          }});
          const payload = await response.json().catch(() => ({{}}));
          if (!response.ok || payload.ok === false) {{
            throw new Error(payload.error || "Could not save the Space_V picture folder.");
          }}
          renderSpaceVPictureSettings(payload);
        }} catch (error) {{
          if (spaceVPictureFolderState) spaceVPictureFolderState.textContent = String(error && error.message ? error.message : error);
        }} finally {{
          spaceVPictureFolderSave.disabled = false;
        }}
      }});
    }}
    if (serverDataManifestRefresh) {{
      serverDataManifestRefresh.addEventListener("click", async () => {{
        ...
        if (serverDataManifestState) serverDataManifestState.textContent = "Applying staged Lesson Vault updates...";
        try {{
          ...
          if (serverDataManifestState) {{
            serverDataManifestState.textContent = payload.mode === "apply-pending"
              ? "Staged builder changes queued for one consolidated publish."
              : `Manifest refreshed: ${{entries.toLocaleString()}} items in ${{folders.toLocaleString()}} folders.`;
          }}
        }} catch (error) {{
    ...
    const DASHBOARD_LOG_VISIBLE_POLL_MS = 10000;
    const DASHBOARD_LOG_HIDDEN_POLL_MS = 60000;
    const dashboardLogTimers = {{ login: 0, learning: 0 }};
    const dashboardLogInflight = {{ login: false, learning: false }};
    const scheduleDashboardLogPoll = (mode, delay = DASHBOARD_LOG_VISIBLE_POLL_MS) => {{
      if (!dashboardLogTimers.hasOwnProperty(mode)) return;
      if (dashboardLogTimers[mode]) window.clearTimeout(dashboardLogTimers[mode]);
      dashboardLogTimers[mode] = window.setTimeout(async () => {{
        if (activeDashboardTab === mode) {{
          await loadDashboardLog(mode, {{ silent: true }});
        }}
        scheduleDashboardLogPoll(mode, document.hidden ? DASHBOARD_LOG_HIDDEN_POLL_MS : DASHBOARD_LOG_VISIBLE_POLL_MS);
      }}, delay);
    }};
    const loadDashboardLog = async (mode, options = {{}}) => {{
      ...
      if (!listNode) return;
      if (dashboardLogInflight[mode]) return;
      dashboardLogInflight[mode] = true;
      const silent = Boolean(options && options.silent);
      if (!silent) listNode.textContent = "Loading...";
      try {{
        ...
        query.set("limit", silent ? "200" : "1000");
        ...
        if (!silent) listNode.textContent = String(error && error.message ? error.message : error);
      }} finally {{
        dashboardLogInflight[mode] = false;
      }}
    }};
    const showDashboardTab = (name) => {{
      ...
      if (activeDashboardTab === "login") {{
        void loadDashboardLog("login");
        scheduleDashboardLogPoll("login");
      }}
      if (activeDashboardTab === "learning") {{
        void loadDashboardLog("learning");
        scheduleDashboardLogPoll("learning");
      }}
    ...
    if (loginLogRefresh) loginLogRefresh.addEventListener("click", () => {{
      void loadDashboardLog("login");
      scheduleDashboardLogPoll("login");
    }});
    if (learningLogRefresh) learningLogRefresh.addEventListener("click", () => {{
      void loadDashboardLog("learning");
      scheduleDashboardLogPoll("learning");
    }});
    ...
    [loginLogDate, loginLogSearch].forEach((node) => node && node.addEventListener("change", () => {{
      void loadDashboardLog("login");
      scheduleDashboardLogPoll("login");
    }}));
    [learningLogDate, learningLogSearch].forEach((node) => node && node.addEventListener("change", () => {{
      void loadDashboardLog("learning");
      scheduleDashboardLogPoll("learning");
    }}));
