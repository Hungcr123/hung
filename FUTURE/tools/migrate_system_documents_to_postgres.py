#!/usr/bin/env python3
"""Migrate selected Server 2 system documents to PostgreSQL."""
...
def where_clause(alias: str = "path") -> str:
    return " OR ".join(f"lower({alias}) like '%\\{name}'" for name in NAMES)
...
def sqlite_rows() -> list[dict]:
    ...
    try:
        ...
        for row in con.execute(
            f"SELECT path_key,path,content,encoding,sha256,file_size,file_mtime_ns,updated_at_utc FROM documents WHERE {where_clause()} ORDER BY path_key"
        ):
...
def pg_rows() -> list[dict]:
    def _read(connection):
        with connection.cursor() as cursor:
            cursor.execute(
                f"SELECT path_key,path,content,encoding,sha256,file_size,file_mtime_ns,updated_at_utc FROM future_server2.documents WHERE {where_clause()} ORDER BY path_key"
            )
            return [
                {
...
def cleanup_test_rows() -> dict:
    def _cleanup(connection):
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM future_server2.documents WHERE path_key like '%codexpgsystem%'")
            deleted = int(cursor.rowcount or 0)
            cursor.execute(f"SELECT count(*), coalesce(sum(file_size),0) FROM future_server2.documents WHERE {where_clause()}")
            row = cursor.fetchone()
        return {"deleted": deleted, "remaining_system_documents": int(row[0] or 0), "remaining_bytes": int(row[1] or 0)}

    return app.postgres_execute(_cleanup)

def purge_policy_excluded_rows() -> dict:
    def _purge(connection):
        with connection.cursor() as cursor:
            cursor.execute(f"DELETE FROM future_server2.documents WHERE {where_clause()}")
            deleted = int(cursor.rowcount or 0)
            cursor.execute(f"SELECT count(*), coalesce(sum(file_size),0) FROM future_server2.documents WHERE {where_clause()}")
            row = cursor.fetchone()
        return {"deleted": deleted, "remaining_system_documents": int(row[0] or 0), "remaining_bytes": int(row[1] or 0)}

    return app.postgres_execute(_purge)


def round_trip() -> dict:
    content = b'{"version":1,"codex":"system"}'
    row = {
        "path_key": "c:\\server data\\codexpgsystem\\_future_frontend_reload.json",
        "path": r"C:\server data\codexpgsystem\_future_frontend_reload.json",
        "content": content,
        "encoding": "utf-8",
...
def parity_result(rt: dict | None = None, cleanup_before: dict | None = None, cleanup_after: dict | None = None, args: argparse.Namespace | None = None) -> dict:
    ...
    return {
        ...
        "policy_excluded_from_generic_pg": bool(args and args.policy_excluded),
        "policy_excluded_ok": bool(args and args.policy_excluded and len(pg_all) == 0),
        "round_trip": rt,
...
def main() -> int:
    ...
    parser.add_argument("--policy-excluded", action="store_true", help="Verify selected system docs are absent from generic PostgreSQL documents.")
    parser.add_argument("--purge-policy-excluded", action="store_true", help="Delete selected system docs from generic PostgreSQL documents.")
    args = parser.parse_args()
    if not os.environ.get("FUTURE_PG_DSN"):
        raise RuntimeError("Set FUTURE_PG_DSN before migrating system documents to PostgreSQL.")
    app.postgres_initialize_schema()
    cleanup_before = cleanup_test_rows() if args.cleanup_test_rows else None
    purge_result = purge_policy_excluded_rows() if args.purge_policy_excluded else None
    cleanup_after = None
    rt = None
    result = {}
    try:
        rows = sqlite_rows()
        if not args.verify_only and not args.dry_run and not args.policy_excluded and not args.purge_policy_excluded:
            for row in rows:
                app.postgres_upsert_document_row(row)
            rt = round_trip()
        result = parity_result(rt, cleanup_before, cleanup_after, args)
        if purge_result is not None:
            result["purge_policy_excluded"] = purge_result
    finally:
        if not args.verify_only and not args.dry_run and not args.policy_excluded and not args.purge_policy_excluded:
    ...
    if args.policy_excluded or args.purge_policy_excluded:
        return 0 if result.get("policy_excluded_ok") else 1
