"""Benchmark Space_W progress reads with 100 users and real `hung` state."""
...
# Added 2026-07-20: exercise dynamic Normal/Train checkpoints without changing production users.
def lesson_paths() -> list[str]:
    rows = []
    for path in (SERVER_DATA / "common").rglob("*.Space_W"):
        try:
        ...
        if len(rows) >= len(USERS):
            return rows
    raise RuntimeError(f"Only {len(rows)} Space_W files were found")
...
def hung_rows() -> list[tuple[str, str]]:
    ...
    try:
        rows = connection.execute(
            "SELECT path,identity FROM lesson_progress WHERE username='hung' AND space='Space_W' ORDER BY updated_at_utc DESC"
        ).fetchall()
    finally:
        connection.close()
    if not rows:
        raise RuntimeError("Real user hung has no Space_W progress rows")
...
def seed_record() -> dict:
    ...
    try:
        row = connection.execute(
            "SELECT record_json FROM lesson_progress WHERE username='hung' AND space='Space_W' ORDER BY length(record_json) DESC LIMIT 1"
        ).fetchone()
    finally:
        connection.close()
    if not row:
        raise RuntimeError("No seed Space_W record exists")
...
def main() -> int:
    ...
    def seed(index: int) -> None:
        ...
        record.update({
            "action": "autosave",
            "path": path,
            "identity": f"codex-space-w-get-{index:03d}",
            "title": Path(path).stem,
            "runId": f"codex-space-w-run-{index:03d}",
            "activeRun": True,
        ...
        response = requests.post(
            f"{BASE}/space-w/progress?client_source=codex_get_seed",
    ...
    def get_one(index: int, etag: str = "") -> tuple:
        ...
        response = requests.get(
            ...
            headers=headers,
            params={"path": paths[index], "identity": f"codex-space-w-get-{index:03d}"},
            timeout=30,
        )
    ...
    if any(row[1] != 304 for row in distinct_conditional):
        raise RuntimeError("One or more distinct-user Space_W conditional reads did not return 304")
    if any(not row[3] for row in hung_result):
        raise RuntimeError("One or more real hung Space_W rows did not resolve")
    if any(row[1] != 304 for row in hung_conditional):
        raise RuntimeError("One or more hung Space_W conditional reads did not return 304")
    print(json.dumps({"space_w_progress_get_hot_path": phases}, ensure_ascii=True, separators=(",", ":")))
    return 0
