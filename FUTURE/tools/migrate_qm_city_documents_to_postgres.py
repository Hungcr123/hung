#!/usr/bin/env python3
"""Migrate QM City/shared-world JSON documents to PostgreSQL."""
...
WHERE = "lower(path) like '%\\_future_qm_city_%' or lower(path) like '%\\_future_shared_world%'"
...
def pg_rows() -> list[dict]:
    def _read(connection):
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT path_key,path,content,encoding,sha256,file_size,file_mtime_ns,updated_at_utc FROM future_server2.documents WHERE lower(path) like '%\\_future_qm_city_%' or lower(path) like '%\\_future_shared_world%' ORDER BY path_key"
            )
            return [
                {
...
def cleanup_test_rows() -> dict:
    def _cleanup(connection):
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM future_server2.documents WHERE path_key like '%codexpgqmcity%'")
            deleted = int(cursor.rowcount or 0)
            cursor.execute("SELECT count(*), coalesce(sum(file_size),0) FROM future_server2.documents WHERE lower(path) like '%\\_future_qm_city_%' or lower(path) like '%\\_future_shared_world%'")
            row = cursor.fetchone()
        return {"deleted": deleted, "remaining_qm_city_documents": int(row[0] or 0), "remaining_bytes": int(row[1] or 0)}

    return app.postgres_execute(_cleanup)


def round_trip() -> dict:
    content = b'{"version":1,"codex":"qmcity"}'
    row = {
        "path_key": "c:\\server data\\codexpgqmcity\\_future_qm_city_training.json",
        "path": r"C:\server data\codexpgqmcity\_future_qm_city_training.json",
        "content": content,
        "encoding": "utf-8",
...
def main() -> int:
    ...
    if not os.environ.get("FUTURE_PG_DSN"):
        raise RuntimeError("Set FUTURE_PG_DSN before migrating QM City/shared-world documents to PostgreSQL.")
