...
function readManifest() {
  ...
  if (!manifest || manifest.version !== 1 || !Array.isArray(manifest.parts)) {
    throw new Error("Unsupported status_page parts manifest.");
  }
  return manifest;
}
...
function buildSource(manifest) {
  return manifest.parts.map((part) => {
    ...
    if (!fs.existsSync(filePath)) {
      throw new Error(`Missing status_page part: ${path.relative(root, filePath)}`);
    }
...
if (checkOnly) {
  if (generated !== existing) {
    console.error(JSON.stringify({
      ok: false,
      error: "03_status_page.py does not match status_page_parts build output.",
