"""Benchmark Space_Q writes across unique users, retries, stale packets, contention, and real state."""
...
import benchmark_space_q_progress_get_hot_path as space_q_get
...
def progress_row(username: str, path: str) -> tuple[int, dict]:
    ...
    try:
        row = connection.execute(
            ...
            "WHERE username=? AND space='Space_Q' AND path=? ORDER BY updated_at_utc DESC LIMIT 1",
            (username, path),
        ).fetchone()
    finally:
        connection.close()
...
def snapshot_hung() -> tuple[list[str], list[tuple], list[str], list[tuple]]:
    ...
    try:
        ...
        progress_rows = connection.execute(
            "SELECT * FROM lesson_progress WHERE username='hung' AND space='Space_Q'"
        ).fetchall()
        namespace_rows = connection.execute(
            "SELECT * FROM lesson_progress_namespaces WHERE username='hung' AND space='Space_Q'"
        ).fetchall()
    finally:
        connection.close()
...
def restore_hung(snapshot: tuple[list[str], list[tuple], list[str], list[tuple]]) -> None:
    ...
    try:
        ...
        connection.execute("DELETE FROM lesson_progress WHERE username='hung' AND space='Space_Q'")
        connection.execute("DELETE FROM lesson_progress_namespaces WHERE username='hung' AND space='Space_Q'")
        if namespace_rows:
...
def next_test_base_time() -> datetime:
    ...
    try:
        rows = connection.execute(
            "SELECT record_json FROM lesson_progress WHERE username LIKE 'codexload%' AND space='Space_Q'"
        ).fetchall()
    finally:
        connection.close()
    for (encoded,) in rows:
...
def main() -> int:
    ...
    paths = space_q_get.lesson_paths()
    template = space_q_get.seed_record()
    ...
    def make_payload(index: int, path: str, identity: str, stamp: datetime, done: int = 0) -> dict:
        ...
        total = max(2, int(record.get("state", {}).get("questionTotal") or record.get("nodeCount") or 20))
        state = record.get("state") if isinstance(record.get("state"), dict) else {}
        state.update({
            "savedAt": saved_at,
            "updatedAt": saved_at,
            "runId": f"codex-space-q-post-run-{run_tag}-{index:03d}",
            "activeRun": True,
            "questionDone": done,
            "questionTotal": total,
            "syncOperationId": f"codex-space-q-operation-{identity}-{saved_at}",
            "complete": False,
    ...
    def post(token: str, payload: dict) -> tuple:
        ...
        response = requests.post(
            f"{BASE}/space-q/progress?client_source=codex_post_benchmark&response=compact-v1",
    ...
    unique_payloads = [
        make_payload(index, paths[index], f"codex-space-q-post-{run_tag}-{index:03d}", base_time + timedelta(seconds=index), index % 5)
    ...
    stale_path = paths[1]
    newer = make_payload(1, stale_path, f"codex-space-q-post-{run_tag}-001", base_time + timedelta(minutes=10), 4)
    post(tokens[USERS[1]], newer)
    common.wait_for_writer_quiescence()
    time.sleep(0.25)
    stale = make_payload(1, stale_path, f"codex-space-q-post-{run_tag}-001", base_time, 1)
    ...
    phases["stale_retry_older_checkpoint"]["newest_question_done"] = int(stale_record.get("state", {}).get("questionDone") or -1)

    contention_path = paths[2]
    contention_payloads = [
        make_payload(2, contention_path, f"codex-space-q-post-{run_tag}-002", base_time + timedelta(seconds=index), index % 10)
        for index in range(100)
    ]
    ...
    hung_path, hung_identity = space_q_get.hung_rows()[0]
    ...
    if any(row[1] != 200 for row in all_rows):
        raise RuntimeError("One or more Space_Q writes failed")
    if any(row[4].get("response_schema") != "space-q-progress-compact-v1" for row in unique_rows + duplicate_rows):
        raise RuntimeError(
            "Accepted/duplicate Space_Q writes did not use compact ACKs: "
    ...
    if phases["same_row_contention"]["final_saved_at"] != phases["same_row_contention"]["expected_saved_at"]:
        raise RuntimeError("Same-row contention lost the newest Space_Q checkpoint")
    print(json.dumps({"space_q_progress_post_hot_path": phases}, ensure_ascii=True, separators=(",", ":")))
    return 0
