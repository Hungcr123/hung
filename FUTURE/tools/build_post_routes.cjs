...
function readManifest() {
  ...
  if (!manifest || manifest.version !== 1 || !Array.isArray(manifest.parts)) {
    throw new Error("Unsupported post route parts manifest.");
  }
  return manifest;
}
...
function buildSource(manifest) {
  return manifest.parts.map((part) => {
    ...
    if (!fs.existsSync(filePath)) {
      throw new Error(`Missing post route part: ${path.relative(root, filePath)}`);
    }
...
if (checkOnly) {
  if (generated !== existing) {
    console.error(JSON.stringify({
      ok: false,
      error: "04_handler_post_routes.py does not match post_route_parts build output.",
