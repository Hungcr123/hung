#!/usr/bin/env python3
"""Migrate Space PDF/Picture AI state documents to PostgreSQL."""
...
WHERE = (
    "lower(path) like '%\\_future_space_pdf_ai_question_progress.json' or "
    "lower(path) like '%\\_future_space_pdf_ai_region_notices.json' or "
    "lower(path) like '%\\_future_space_pdf_ai_region_questions.json' or "
    "lower(path) like '%\\_future_space_pdf_audio_markers.json'"
)
...
def pg_rows() -> list[dict]:
    def _read(connection):
        with connection.cursor() as cursor:
            cursor.execute(
                """
                SELECT path_key,path,content,encoding,sha256,file_size,file_mtime_ns,updated_at_utc
                FROM future_server2.documents
                WHERE lower(path) LIKE '%\\_future_space_pdf_ai_question_progress.json'
                   OR lower(path) LIKE '%\\_future_space_pdf_ai_region_notices.json'
                   OR lower(path) LIKE '%\\_future_space_pdf_ai_region_questions.json'
                   OR lower(path) LIKE '%\\_future_space_pdf_audio_markers.json'
                ORDER BY path_key
                """
            )
            return [
...
def cleanup_test_rows() -> dict:
    def _cleanup(connection):
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM future_server2.documents WHERE path_key like '%codexpgpdfai%'")
            ...
            row = cursor.fetchone()
        return {"deleted": deleted, "remaining_space_pdf_ai_documents": int(row[0] or 0), "remaining_bytes": int(row[1] or 0)}

    return app.postgres_execute(_cleanup)


def round_trip() -> dict:
    content = b'{"version":1,"documents":{"codexpgpdfai":{"ok":true}}}'
    row = {
        "path_key": "c:\\server data\\codexpgpdfai_future_space_pdf_ai_region_notices.json",
        "path": r"C:\server data\codexpgpdfai_future_space_pdf_ai_region_notices.json",
        "content": content,
        "encoding": "utf-8",
...
def main() -> int:
    ...
    if not os.environ.get("FUTURE_PG_DSN"):
        raise RuntimeError("Set FUTURE_PG_DSN before migrating Space PDF AI documents to PostgreSQL.")
