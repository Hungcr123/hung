...
const sourcePath = path.join(root, "FUTURE", "server_parts", "http_server", "04_handler_post_routes.py");
const partsRoot = path.join(root, "FUTURE", "server_parts", "http_server", "post_route_parts");
const manifestPath = path.join(partsRoot, "manifest.json");

const partSpecs = [
  ["01_entry_dashboard_admin.pyfrag", ""],
  ["02_pdf_ocr_scan.pyfrag", "    if path == \"/pdf/ocr\":"],
  ["03_pdf_ai_tasks_auth.pyfrag", "    if path == \"/ai-agent/history\":"],
  ["04_chat_stream_screen_paint.pyfrag", "    if path == \"/chat/upload\":"],
  ["05_vocab_progress_leaderboard.pyfrag", "    if path == \"/vocab/scan-space-w\":"],
  ["06_world_battle_game_lesson.pyfrag", "    if path == \"/world/move\":"],
  ["07_transcribe_upload.pyfrag", "    if path != \"/transcribe\":"],
];
...
function assertFreshPartsFolder() {
  ...
  if (existing.length) {
    throw new Error(`Refusing to overwrite existing post_route_parts files. Remove or archive ${path.relative(root, partsRoot)} first.`);
  }
}
...
function firstRoute(text) {
  const match = text.match(/^\s+if path (?:==|in) [^\n]+/m);
  return match ? match[0].trim() : "def do_POST(self)";
}

function lastRoute(text) {
  const matches = [...text.matchAll(/^\s+if path (?:==|!=|in) [^\n]+/gm)];
...
if (rebuilt !== source) {
  throw new Error("Internal split check failed: rebuilt 04_handler_post_routes.py does not match source.");
}

const manifest = {
  version: 1,
  source: "FUTURE/server_parts/http_server/04_handler_post_routes.py",
  mode: "post_route_method_fragments",
...
console.log(JSON.stringify({
  ...
  parts: parts.map((part) => ({
    file: path.join("FUTURE", "server_parts", "http_server", "post_route_parts", part.file),
