"""HTTP retry regression for durable shared paragraph operation IDs."""
...
LEGACY_WAL = Path(r"C:\QMLearn\users\codexload001\_future_space_p_progress.json.wal.jsonl")
...
def database_row() -> tuple[int, dict]:
    ...
    try:
        row = connection.execute(
            ...
            "WHERE username=? AND space='Space_P' ORDER BY updated_at_utc DESC LIMIT 1",
            (USERNAME,),
        ).fetchone()
    finally:
        connection.close()
    if not row:
        raise RuntimeError("Run benchmark_space_p_progress_post_hot_path.py first")
    return int(row[0] or 0), json.loads(row[1])


# Added 2026-07-21: an ACK-lost P/L/S retry stays a zero-write compact ACK.
def main() -> int:
    ...
    if payload.get("response_schema") != "space-p-progress-compact-v1":
        raise RuntimeError("Duplicate paragraph retry did not receive compact ACK")
    if revision_after != revision_before:
        raise RuntimeError("Duplicate paragraph retry increased the durable revision")
    if int(writer_after.get("tasks", 0) or 0) != int(writer_before.get("tasks", 0) or 0):
        raise RuntimeError("Duplicate paragraph retry created a SQLite writer task")
    if legacy_after != legacy_before:
        raise RuntimeError("Duplicate paragraph retry wrote the retired JSONL WAL")
    if str(stored.get("syncOperationId") or "") != operation_id:
        raise RuntimeError("Stored paragraph operation ID changed during retry")
    print(f"space_p_progress_post_http=ok revision={revision_after} writer_tasks=0 legacy_wal_growth=0 compact_bytes={len(response.content)} space={stored.get('space')}")
    return 0
