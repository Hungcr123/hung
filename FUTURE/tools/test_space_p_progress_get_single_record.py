"""Regression for single-record shared Space_P/L/S progress reads."""
...
# Added 2026-07-21: keep P/L/S GET on one RAM record and serialize only after ETag validation.
def main() -> int:
    source = SOURCE.read_text(encoding="utf-8")
    start = source.index("def read_space_p_progress(")
    end = source.index("\ndef space_p_progress_etag", start)
    ...
    assert "def space_p_progress_etag(" in source

    route = ROUTE.read_text(encoding="utf-8")
    route_start = route.index('if path == "/space-p/progress":')
    route_end = route.index('if path == "/vocab/registry":', route_start)
    progress_route = route[route_start:route_end]
    assert progress_route.index("space_p_progress_etag") < progress_route.index("json_bytes({")
    assert 'self.headers.get("If-None-Match"' in progress_route
    assert '"X-Future-Cache-Hit": "space-p-progress-etag"' in progress_route
    print("space_p_progress_get_single_record=ok raw_key_first=true namespace_clone=false fallback=true etag_preflight=true")
    return 0
