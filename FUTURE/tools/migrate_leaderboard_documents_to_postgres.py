#!/usr/bin/env python3
"""Migrate selected leaderboard/activity JSON documents to PostgreSQL."""
...
WHERE = "lower(path) like '%leaderboard%' or lower(path) like '%space_leaderboard%'"
...
def pg_rows() -> list[dict]:
    def _read(connection):
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT path_key,path,content,encoding,sha256,file_size,file_mtime_ns,updated_at_utc FROM future_server2.documents WHERE lower(path) like '%leaderboard%' or lower(path) like '%space_leaderboard%' ORDER BY path_key"
            )
            return [
                {
...
def public_row(row: dict) -> dict:
    content = bytes(row["content"] or b"")
    stable = {k: v for k, v in row.items() if k not in {"content", "updated_at_utc", "file_mtime_ns"}}
    return stable | {"content_sha256": hashlib.sha256(content).hexdigest(), "content_len": len(content)}
...
def cleanup_test_rows() -> dict:
    def _cleanup(connection):
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM future_server2.documents WHERE path_key like 'codexpg%'")
            deleted = int(cursor.rowcount or 0)
            cursor.execute("SELECT count(*) FROM future_server2.documents WHERE lower(path) like '%leaderboard%' or lower(path) like '%space_leaderboard%'")
            remaining = int(cursor.fetchone()[0] or 0)
        return {"deleted": deleted, "remaining_selected_documents": remaining}
    return app.postgres_execute(_cleanup)


def round_trip() -> dict:
    content = b'{"ok":true,"source":"codex"}'
    first = app.postgres_upsert_document_row({
        "path_key": "codexpg\\leaderboard_test.json",
        "path": r"C:\server data\codexpg_leaderboard_test.json",
        "content": content,
        "encoding": "utf-8",
    ...
    second = app.postgres_upsert_document_row({
        "path_key": "codexpg\\leaderboard_test.json",
        "path": r"C:\server data\codexpg_leaderboard_test.json",
        "content": content,
        "encoding": "utf-8",
...
def main() -> int:
    ...
    if not os.environ.get("FUTURE_PG_DSN"):
        raise RuntimeError("Set FUTURE_PG_DSN before migrating documents to PostgreSQL.")
