#!/usr/bin/env python3
"""Migrate viewer/tool bridge state documents to PostgreSQL."""
...
WHERE = (
    "lower(path) like '%_pdf_viewer_state%' or "
    "lower(path) like '%_main_pdf_tool_state%' or "
    "lower(path) like '%_side_viewer_bridge%' or "
    "lower(path) like '%_image_viewer_state%'"
)
...
def pg_rows() -> list[dict]:
    def _read(connection):
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT path_key,path,content,encoding,sha256,file_size,file_mtime_ns,updated_at_utc FROM future_server2.documents WHERE lower(path) like '%_pdf_viewer_state%' or lower(path) like '%_main_pdf_tool_state%' or lower(path) like '%_side_viewer_bridge%' or lower(path) like '%_image_viewer_state%' ORDER BY path_key"
            )
            return [
                {
...
def cleanup_test_rows() -> dict:
    def _cleanup(connection):
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM future_server2.documents WHERE path_key like '%codexpgviewer%'")
            deleted = int(cursor.rowcount or 0)
            cursor.execute(
                "SELECT count(*), coalesce(sum(file_size),0) FROM future_server2.documents WHERE lower(path) like '%_pdf_viewer_state%' or lower(path) like '%_main_pdf_tool_state%' or lower(path) like '%_side_viewer_bridge%' or lower(path) like '%_image_viewer_state%'"
            )
            row = cursor.fetchone()
        return {
            "deleted": deleted,
            "remaining_viewer_tool_documents": int(row[0] or 0),
            "remaining_bytes": int(row[1] or 0),
        }
...
def round_trip() -> dict:
    content = b'{"version":1,"codex":"viewer-tool"}'
    row = {
        "path_key": "users\\codexpgviewer_pdf_viewer_state.json",
        "path": r"C:\QMLearn\users\codexpgviewer_pdf_viewer_state.json",
        "content": content,
        "encoding": "utf-8",
...
def main() -> int:
    ...
    if not os.environ.get("FUTURE_PG_DSN"):
        raise RuntimeError("Set FUTURE_PG_DSN before migrating viewer/tool documents to PostgreSQL.")
