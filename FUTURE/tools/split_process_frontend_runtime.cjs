...
const sourcePath = path.join(root, "FUTURE", "server_parts", "06_process_frontend_runtime.py");
const partsRoot = path.join(root, "FUTURE", "server_parts", "process_frontend_runtime");

const partSpecs = [
  ["01_model_paths.py", "def normalize_words(text: str) -> list[str]:"],
  ["02_tunnel_process_dashboard.py", "def cloudflared_path() -> Path:"],
  ["03_local_stt_runtime.py", "def cpu_thread_count() -> int:"],
  ["04_stt_worker_bridge.py", "def stt_worker_request(path: str, payload: dict | None = None, timeout: float = 8.0, method: str = \"POST\") -> dict:"],
  ["05_payload_assets_storage.py", "def json_bytes(payload: dict) -> bytes:"],
  ["06_frontend_delivery.py", "def cached_public_file_bytes(path: Path) -> bytes:"],
];

const source = fs.readFileSync(sourcePath, "utf8");
if (source.includes("PROCESS_FRONTEND_PART_FILES = (")) {
  throw new Error("06_process_frontend_runtime.py already appears to use nested runtime parts.");
}
...
console.log(JSON.stringify({
  ...
  parts: parts.map(([fileName, body]) => ({
    file: path.join("FUTURE", "server_parts", "process_frontend_runtime", fileName),
