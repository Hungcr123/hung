#!/usr/bin/env python3
"""Migrate AI agent and word-agent history documents to PostgreSQL."""
...
WHERE = "lower(path) like '%\\_future_ai_agent_history.json' or lower(path) like '%\\_future_word_agent_history.json'"
...
def pg_rows() -> list[dict]:
    def _read(connection):
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT path_key,path,content,encoding,sha256,file_size,file_mtime_ns,updated_at_utc FROM future_server2.documents WHERE lower(path) like '%\\_future_ai_agent_history.json' or lower(path) like '%\\_future_word_agent_history.json' ORDER BY path_key"
            )
            return [
                {
...
def cleanup_test_rows() -> dict:
    def _cleanup(connection):
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM future_server2.documents WHERE path_key like '%codexpgaihist%'")
            deleted = int(cursor.rowcount or 0)
            cursor.execute(
                "SELECT count(*) FROM future_server2.documents WHERE lower(path) like '%\\_future_ai_agent_history.json' or lower(path) like '%\\_future_word_agent_history.json'"
            )
            remaining = int(cursor.fetchone()[0] or 0)
        return {"deleted": deleted, "remaining_ai_history_documents": remaining}

    return app.postgres_execute(_cleanup)


def round_trip() -> dict:
    content = b'{"version":1,"history":[{"id":"codexpgaihist","title":"Codex","english":"ok","vietnamese":"ok","created_at":"2026-07-25T00:00:00Z","updated_at":"2026-07-25T00:00:00Z"}],"updated_at":"2026-07-25T00:00:00Z"}'
    row = {
        "path_key": "users\\codexpgaihist\\_future_ai_agent_history.json",
        "path": r"C:\server data\users\codexpgaihist\_future_ai_agent_history.json",
        "content": content,
        "encoding": "utf-8",
...
def main() -> int:
    ...
    if not os.environ.get("FUTURE_PG_DSN"):
        raise RuntimeError("Set FUTURE_PG_DSN before migrating AI history documents to PostgreSQL.")
