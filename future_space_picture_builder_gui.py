"""Tkinter GUI for building canonical Space Picture packages."""
...
PICTURE_FILE_TYPES = [
    ("Picture files", "*.png *.jpg *.jpeg *.webp *.bmp *.gif"),
    ("All files", "*.*"),
]


class SpacePictureBuilderGui:
    """Build one thin .space_picture package from a canonical picture asset."""

    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("Future Space Picture Builder")
        ...
        self.status_var = tk.StringVar(value="Choose a picture to begin.")
    ...
    def _build_ui(self) -> None:
        ...
        rows = [
            ("Source picture", self.source_var, self._choose_picture, "Open picture"),
        ...
        self.build_button.grid(row=5, column=0, columnspan=3, sticky="ew", pady=12)

        result = ttk.LabelFrame(frame, text="Verified result", padding=12)
        result.grid(row=6, column=0, columnspan=3, sticky="nsew", pady=10)
    ...
    def _choose_picture(self) -> None:
        selected = filedialog.askopenfilename(title="Choose picture", filetypes=PICTURE_FILE_TYPES)
        if not selected:
            return
        ...
        self.output_name_var.set(f"{source.stem}.space_picture")
    ...
    def _start_build(self) -> None:
        ...
        output = Path(self.output_dir_var.get().strip()) / output_name
        if not source.is_file():
            messagebox.showerror("Space Picture build failed", "Choose an existing picture file.")
            return
        if not output_name:
            messagebox.showerror("Space Picture build failed", "Enter an output filename.")
            return
        if output.exists() and not messagebox.askyesno("Replace package", "The output exists. Replace it?"):
            return
        self.build_button.configure(state="disabled")
        self.status_var.set("Importing the canonical asset and verifying the thin package...")

        def worker() -> None:
            build_session = builder_server2_build_begin("Space_Picture", output)
            build_success = False
            try:
                result = build_space_picture_package(
                    source,
                    output,
                ...
                builder_server2_build_end(build_session, "Space_Picture", output, build_success)

        threading.Thread(target=worker, daemon=True, name="space-picture-builder").start()

    def _show_result(self, result) -> None:
        self.build_button.configure(state="normal")
        self.status_var.set("Canonical asset and thin package verified.")
    ...
    def _show_error(self, message: str) -> None:
        ...
        messagebox.showerror("Space Picture build failed", message)
    ...
    def _reset(self) -> None:
        for variable in (
            ...
            self.lesson_id_var,
            self.document_id_var,
            self.output_var,
            self.size_var,
        ):
            variable.set("")
        self.status_var.set("Choose a picture to begin.")


# Added 2026-07-22: expose Picture imports through the same GUI workflow as PDF.
def main() -> int:
    root = tk.Tk()
    SpacePictureBuilderGui(root)
    root.mainloop()
    return 0
